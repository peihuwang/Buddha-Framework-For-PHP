...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return boolean
     */
    protected function setCache($type,$son,$appid,$appsecret,$access_token,$expire){

        $num = $this->db->countRecords( 'wechatconfig', "type='{$type}'
          and  son='{$son}'
          and  appid='{$appid}'
        ...
        if($num==0){
            ...
            $this->db->addRecords ( $data, 'wechatconfig' );
        }else{
            $nowtime =time();
            $data = array();
            $data['accesstoken']=$access_token;
            $data['expiretime']= $nowtime+$expire;
            $data['expiretimestr']= date('Y-m-d H:i:s', $data['expiretime']);
            $this->db->updateRecords( $data, "wechatconfig","type='{$type}'
          and  son='{$son}'
          and  appid='{$appid}'
          and  appsecret='{$appsecret}'
        " );
     ...
     * @return mixed
     */
    protected function getCache($type,$son,$appid,$appsecret){
        ...
        if($num==0){
           ...
           $Db_wechatconfig=$this->db->getSingleFiledValues ( '', 'wechatconfig', "type='{$type}'
          and  son='{$son}'
          and  appid='{$appid}'
          and  appsecret='{$appsecret}'
        " );

            $expiretime = $Db_wechatconfig['expiretime'];
            if ($expiretime < time()){
                return false;
            }else{
                $accesstoken= $Db_wechatconfig['accesstoken'];
                return $accesstoken;
            }
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $type = 'wechat';
        $son ='token';
        if ($rs = $this->getCache($type,$son,$this->appid,$this->appsecret))  {
            $this->access_token = $rs;
            return $rs;
        }

        $result = $this->http_get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$appid.'&secret='.$appsecret);
        self::API_URL_PREFIX.self::AUTH_URL.'appid='.$appid.'&secret='.$appsecret;
        ...
        if ($result)
        {
            ...
            $this->setCache($type,$son,$this->appid,$this->appsecret,$this->access_token,$expire);
            return $this->access_token;
        }
        return false;
    }
    ...
    public function getJsTicket($appid='',$jsapi_ticket=''){
        ...
        if ($result)
        {
            ...
            $this->setCache($type,$son,$this->appid,$this->appsecret,$this->jsapi_ticket,$expire);
            return $this->jsapi_ticket;
        }
        return false;
    }
