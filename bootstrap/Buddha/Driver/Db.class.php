...
class Buddha_Driver_Db
{
	...
	public  function countRecords($table, $condition = '') {
		$table = str_replace($this->getPrefix(), '', $table);
		$_sql= "SELECT count(*) num FROM {$this->getPrefix()}{$table}";
		if ($condition) {
			$_sql .= " WHERE {$condition}";
		}


		$data = $this->query($_sql)->fetchAll();
	...
	public function updateRecords($field = array(), $table, $condition = '') {
		...
		$_sql = "UPDATE {$this->getPrefix()}{$table} SET ";
	...
	public function addRecords($item = array(), $table) {
		$table = str_replace($this->getPrefix(), '', $table);
		$field = $fieldValue = '';
		$_sql = "INSERT INTO {$this->getPrefix()}{$table} (%s)
			VALUES(%s)";
		if ($item && is_array($item)) {
			...
			$_sql = sprintf($_sql, $field, $fieldValue);
		}
	...
	public function delRecords($table, $condition = '') {
		$table = str_replace($this->getPrefix(), '', $table);
		$_sql= "DELETE FROM {$this->getPrefix()}{$table}";
		if ($condition) {
			$_sql .= " WHERE {$condition}";
		}
	...
	public function getSingleFiledValues($field = array(), $table, $condition = '') {
		...
		$_sql = "SELECT {$field} FROM {$this->getPrefix()}{$table}";
		if ($condition) {
			$_sql .= " WHERE $condition";
		}
		...
		$data = $this->query($_sql)->fetchAll(PDO::FETCH_ASSOC);
		return $data[0];
	}
	...
	public function getFiledValues($field = array(), $table, $condition = '')
	{
		...
		return  $data = $this->query($_sql)->fetchAll(PDO::FETCH_ASSOC);
	}
