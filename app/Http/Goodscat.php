<?php
class Goodscat extends  Buddha_App_Model{
    ...
    public function goods_cat($sub=0){//获取一级分类
        $goods_cat=$this->getFiledValues('',"isdel=0 and sub={$sub} AND id<>1 order by id ASC, view_order ASC");
        return $goods_cat;
    }
    public function goods_cats($sub=0){//获取一级分类
        $goods_cat=$this->getFiledValues('',"isdel=0 and sub={$sub} order by id ASC, view_order ASC");
        return $goods_cat;
    }
        ...
        foreach ( $cates as $k => $v ) {
            if ($v ['sub'] == $cid) {
                ...
                $table .= '<tr  pid="'.$v ['sub'] .'" cid="'. $v ['id'] .'" depath="'.$level .'">
                  ...
                  <td>'.$isopen.'</td>
                    <td>';
                $table .= '<a title="添加子类" href="index.php?a=addcat&c=goods&cid=' . $v ['id'] . '">[添加子类]</a>
                       <a title="编辑" href="index.php?a=editcat&c=goods&id=' . $v ['id'] . '">[编辑]</a>
                       <a onclick="return delnav();" href="index.php?a=delcat&c=goods&id=' . $v ['id'] . '" title="删除">[删除]</a>';
                $table .= '</td></tr> ';
...
/**得到产品的单位名称的option**/
    ...
    public function delGelleryimage($goodscat_id){
       $goodimg= $this->getFiledValues('',"id='{$goodscat_id}'");
        if(is_array($goodimg) and count($goodimg)){
          foreach($goodimg as $k=>$v){
              $this->del($v['id']);
              @unlink(PATH_ROOT . $v ['class_img']);
          }
        }
    }

    public function pcaddimage($dataImg = array(),$cart_id,$actionval='goodscat'){
        foreach($dataImg as $k=>$v){
            if($v){
                ...
                exit;*/
                //$this->resolveImageForRotate(PATH_ROOT.$Image,NULL);这是为他
                
                if($Image){
                    Buddha_Tool_File::thumbImageSameWidth(PATH_ROOT . $Image, 320, 320, 'S_');
                }

                $sourcepic = str_replace("storage/{$actionval}/",'',$Image);
                $data['class_img'] = "storage/{$actionval}/S_" . $sourcepic;
                @unlink(PATH_ROOT."storage/{$actionval}/" . $sourcepic);
                $this->updateRecords($data,"id='{$cart_id}'");
            }
