...
class LeaseController extends Buddha_App_Action{
    ...
    public function index(){
        ...
        /**************************/
        $subid=$LeasecatObj->getSingleFiledValues(array('id','sub','cat_name'),"isdel=0 and id='{$cid}'");
        $catlist=$LeasecatObj->getcatist($cid);
        ...
        $strPages = Buddha_Tool_Page::multLink ( $page, $rcount, 'index.php?a='.__FUNCTION__.'&c=lease'.$link.'&', $pagesize );

        //推荐店铺
        $listhot = $this->db->getFiledValues ($fields,  $this->prefix.'lease',"isdel=0 and is_sure=1 and buddhastatus=0 and is_rec=1 {$locdata['sql']} order by click_count DESC  limit 0,10");
    ...
    public function detailed(){
        ...
        $fields =array('*');
        $listhot = $this->db->getFiledValues ($fields,  $this->prefix.'lease',"isdel=0 and is_sure=1 and buddhastatus=0 and is_rec=1 {$locdata['sql']}   order by click_count DESC  limit 0,6");
        $cat=$LeasecatObj->goods_thumbgoods_thumb($lease['leasecat_id']);
        if($cat){
            $header='';
            foreach($cat as $k=>$v){
        ...
        $shop=$ShopObj->getSingleFiledValues('',"id='{$lease['shop_id']}'");
...
////////////////////////////////////////////////////////////////////////////////////////
        $lease['lng']=$shop['lng'];
        $lease['lat']=$shop['lat'];
        $lease['is_verify']=$shop['is_verify'];
        $lease['shop_name']=$shop['name'];
        $lease['mobile']=$resstr;
        $lease['address']=$area.$shop['specticloc'];
        $data=array();
        $data['click_count']=$lease['click_count']+1;
    ...
    public function infosee(){
        ...
        if($Db_orderunm){
            $Db_shop=$LeaseObj->getSingleFiledValues(array('mobile'),"id='{$good_id}' and isdel=0");
            $jsondata = array();
