...
class ShopController extends Buddha_App_Action{
    ...
    public function index(){
        ...
        if($act=='list'){
        $where = " (isdel=0 or isdel=4) and addshopuid='{$uid}'";
      ...
      /*  $pcount = ceil($rcount/$pagesize);
        if($page > $pcount){
            ...
            $list = $this->db->getFiledValues (array('id','name','small','is_sure','createtimestr','createtime','state','number','isdel'),  $this->prefix.'shop', $where . $orderby . Buddha_Tool_Page::sqlLimit ( $page, $pagesize ) );
    ...
    public function add(){
        ...
        if(Buddha_Http_Input::isPost()){
            ...
            $datas=array();
            $data=array();
            $data['username']=$mobile;//手机号
            $password='123456';//密码
            $data['password']=Buddha_Tool_Password::md5($password);//md5密码
            $data['codes']=$password;
            //$data['addshopuid'] = $uid;//添加店铺者的id
            //$data['partnerrate'] = (int)$UserInfo['partnerrate'];
            $data['mobile'] = $mobile;
            $data['mobile_ide'] = 1;
            $data['groupid'] = 1;
            $data['to_group_id'] = '4,1';
            $data['state'] = 1;
            ...
            if($User_insertid){
                ...
                if($shop_id){
                    $Image = Buddha_Http_Upload::getInstance()->setUpload(PATH_ROOT . "storage/shop/{$shop_id}/",array('gif', 'jpg', 'jpeg', 'png'), Buddha::$buddha_array['upload_maxsize'])->run('Image')->getOneReturnArray();
                    if ($Image){
                    ...
                    $datass['isok']='true';
                    $datass['data']='店铺添加成功';
                    $datass['url']='index.php?a=index&c=shop';
                }else{
                    $datass['isok']='false';
                    $datass['data']='服务器忙';
                    $datass['url'] = 'index.php?a=index&c=shop';
