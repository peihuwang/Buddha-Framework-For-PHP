...
class SingleinformationController extends Buddha_App_Action{
    ...
    public function index(){
        ...
        $rcount = $this->db->countRecords( $this->prefix.'singleinformation', $where);
        ...
        $fields =array('id','user_id','shop_id','name','singleinformation_thumb','buddhastatus','is_sure','remarks','isdel','is_hot','is_rec');
...
///////////////////////////////////////////////
        ...
        $id=(int)Buddha_Http_Input::getParameter('id');
        $user_id=(int)Buddha_Http_Input::getParameter('user_id');
        $Db_Supply = $SingleinformationObj->fetch($id);
...
///////////////////////////////////////////////
        if($view==0){
            if($Db_Supply['isdel']==0){
                $SingleinformationObj->updateRecords(array('isdel' => 4), " isdel=0  and  user_id='{$user_id}' and id='{$id}'");
                Buddha_Http_Head::redirect("你将商家的{$title}下架成功!","index.php?a=index&c={$c}");
            }else if($Db_Supply['isdel']==4){
                $SingleinformationObj->updateRecords(array('isdel' =>0), "isdel=4 and  user_id='{$user_id}' and id='{$id}'");
                Buddha_Http_Head::redirect("你将商家的{$title}上架成功!","index.php?a=index&c={$c}");
            }
        }elseif ($view==1){//推荐
            if($Db_Supply['is_rec']==1){
                $SingleinformationObj->updateRecords(array('is_rec' => 0), " isdel=0 and  user_id='{$user_id}' and id='{$id}'");
                Buddha_Http_Head::redirect("你将商家{$title}取消推荐成功！","index.php?a=index&c={$c}");
            }else if($Db_Supply['is_rec']==0){
                $SingleinformationObj->updateRecords(array('is_rec' => 1), " isdel=0 and  user_id='{$user_id}' and id='{$id}'");
                Buddha_Http_Head::redirect("你将商家{$title}设为推荐成功！","index.php?a=index&c={$c}");
            }
        }elseif ($view==2){//热门
            if($Db_Supply['is_hot']==1){
                $SingleinformationObj->updateRecords(array('is_hot' => 0), " isdel=0 and  user_id='{$user_id}' and id='{$id}'");
                Buddha_Http_Head::redirect("你将商家{$title}取消热门成功！","index.php?a=index&c={$c}");
            }else if($Db_Supply['is_hot']==0){
                $SingleinformationObj->updateRecords(array('is_hot' => 1), " isdel=0 and  user_id='{$user_id}' and id='{$id}'");
