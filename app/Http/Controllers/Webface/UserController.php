...
class UserController extends Buddha_App_Action
{
    ...
    public function resetPassword()
    {
        ...
        $data['codes'] = $password;
        $data['mobile_ide'] = 1;
        $UserObj->updateRecords($data, "isdel=0 and mobile='{$mobile}'   ");
        $jsondata = array();

        $Db_User = $UserObj->getSingleFiledValues(array('id', 'usertoken'), "isdel=0 and mobile='{$mobile}' and mobile_ide=1  ");
        $jsondata['user_id'] = $Db_User['id'];
        $jsondata['usertoken'] = $Db_User['usertoken'];
        Buddha_Http_Output::makeWebfaceJson($jsondata, '/webface/?Services=' . $_REQUEST['Services'], 0, '密码重设成功');
    ...
    public function login()
    {
        ...
        $fields = array('id', 'usertoken', 'mobile', 'realname', 'logo', 'groupid', 'password');
        if (strlen($password)) {
            ...
            $usertoken = $UserObj->birthUserToken($Db_User['id'] . ':' . $Db_User['password']);
    ...
    public function register()
    {
        ...
        if (Buddha_Http_Input::checkParameter(array('mobile', 'mobilecode', 'password', 'repassword', 'webface_access_token'))) {
            Buddha_Http_Output::makeWebfaceJson(null, '/webface/?Services=' . $_REQUEST['Services'], 444444, '必填信息没填写');
        }
        ...
        $data['onlineregtime'] = Buddha::$buddha_array['buddha_timestamp'];
        $user_id = $UserObj->add($data);
        $usertoken = $UserObj->birthUserToken($user_id . ':' . $data['password']);
        $UserObj->edit(array('usertoken' => $usertoken), $user_id);
        ...
        $jsondata['user_id'] = $user_id;
        $jsondata['usertoken'] = $usertoken;
        Buddha_Http_Output::makeWebfaceJson($jsondata, '/webface/?Services=' . $_REQUEST['Services'], 0, '用户注册');
    }
    ...
    public function viewprofile()
    {
        ...
        $user_id = $Db_userInfo['user_id'];

        $Db_Userbasic = $UserbasicObj->getSingleFiledValues('', "user_id='{$user_id}' ");
        $albumcover = $Db_Userbasic['albumcover'];
        $jsondata = array();
        $jsondata = $Db_userInfo[0];
        $jsondata['albumcover'] = Buddha_Atom_String::getApiFileUrlStr($albumcover);

        $jsondata['nickname'] = $UserObj->getUserNickName($Db_userInfo[0]);

        $logo = Buddha_Atom_String::getUserLogo($Db_userInfo[0]['logo']);
        $jsondata['logo'] = $logo;
