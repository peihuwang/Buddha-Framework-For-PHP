<?php
class Recruitcat extends  Buddha_App_Model{
     ...
     * 通过需求分类ID获取分类名称
     */
    public function getRecruitcatNameByRecruitcatid($recruitcat_id) {
        if($recruitcat_id>0){
            $Db_Recruitcat=$this->getFiledValues(array('id','sub','cat_name'),' ifopen=0 ');
            if(Buddha_Atom_Array::isValidArray($Db_Recruitcat)){
                foreach ($Db_Recruitcat as $k=>$v){
                    if($v['id']==$recruitcat_id){
                        return $v['cat_name'];
                    }
                }
        ...
        foreach ( $cates as $k => $v ) {
            if ($v ['sub'] == $cid) {
                ...
                $table .= '<tr  pid="'.$v ['sub'] .'" cid="'. $v ['id'] .'" depath="'.$level .'">
                  ...
                  <td>'.$isopen.'</td>
					<td>';
                $table .= '<a title="添加子类" href="index.php?a=add&c=recruitcat&cid=' . $v ['id'] . '">[添加子类]</a>
			           <a title="编辑" href="index.php?a=edit&c=recruitcat&id=' . $v ['id'] . '">[编辑]</a>
			           <a onclick="return delnav();" href="index.php?a=del&c=recruitcat&id=' . $v ['id'] . '" title="删除">[删除]</a>';
                $table .= '</td></tr> ';
    ...
    public function getRecruitcatlist($sub){
        $Db_Recruit= $this->db->getFiledValues(array('id','sub','cat_name','child_count'), $this->table, "sub='{$sub}' and isdel=0");
        return $Db_Recruit;
    }
    ...
    public function getDivRelation($cates, &$table, $cid = 0, $number = '') {
        ...
        foreach ( $cates as $k => $v ) {
            if ($v ['sub'] == $cid) {
                $haschild = 0;$name="<li data-href='index.php?a=index&c=recruit&cid={$v['id']}'>{$v['cat_name']}</li";
                foreach ( $cates as $k1 => $v1 ) {
                    if($v['id']==$v1['sub']){
                        $haschild = 1;
                        $name="<li><span data-href='index.php?a=index&c=recruit&cid={$v['id']}'>{$v['cat_name']}</span>";
                    }
                }
                $table .="{$name}";
                if($haschild)
