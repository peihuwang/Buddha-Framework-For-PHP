...
$.fn.leaseajax = function(url){
		...
		var act='list';
			...
			//cache:false,
				success: function (o) {
					$('#showLoading').hide();
					if(o.isok=='true'){
						var jsonListObj = o.list;
						insertListDiv(jsonListObj);
					}else{
						$('.div_null').html(o.list).removeClass('hide');
					}
					},
				beforeSend: function () {
				},
    ...
    function insertListDiv(json) {
		...
		for (var i = 0; i< json.length; i++) {
            ...
            html+='<div class="goods_img"><span class="'+json[i].is_sure+'"></span><img src="/'+json[i].lease_thumb+'" alt=""></div>';
			html+='<div class="supply_c">';
			html+='<h2>'+json[i].lease_name+'</h2>';
			html+='<div class="Price"><i></i>租金：<b>￥<em>'+json[i].rent+'</em></b></div>';
			html+='<div class="goods_sn"><i></i>类别：'+json[i].cat_name+'</div>';
			html+='</div>';
            if (json[i].is_sure == 4) {
                html += '<div class="eidt"><span data-href="index.php?a=edit&c=lease&id=' + json[i].id + '"><i></i>编辑</span><span onclick="fail(' + json[i].id + ')" style="text-align: center; line-height: 30px">审核失败</span></div>';
            }else {
...
function fail(id) {
    $.get('index.php?a=fail&c=lease',{id:id},function (re) {
        if(re.isok==0){
