...
$.fn.ajaxshiolist = function(url){
    ...
    function insertListDiv(json) {
        ...
        for (var i = 0; i< json.length; i++) {
            ...
            html+='<div class="itemimg"><span class="'+json[i].is_sure+'"></span><img src="/'+json[i].images+'"></div>';
            ...
            html+='<p>开通时间:'+json[i].createtime+'</p>';
            html+='</div>';
            html+=' <div class="oper">';
            // html+='<span data-href="index.php?a=edit&c=shop&id='+json[i].id+'"><button style="width: 50px;"> 审 核</button></span>';
            // html+='  <span class="stop" id="state_'+json[i].id+'" data-u="'+json[i].user_id+'" data-value="'+json[i].id+'" data-a="isdel" data-c="shop">'+json[i].state+'</span>';
/////////////////////////////////////////////////////
            if(json[i].is_sure == "not" ){
                html+='<span data-href="index.php?a=edit&c=shop&id='+json[i].id+'"> 审 核</span>';
...
function auditor(){
    ...
    var partnerrate=$('input[name="partnerrate"]').val(),
        is_sure=$('input:radio[name="is_sure"]:checked').val(),
         remarks=$('textarea[name="remarks"]').val(),
        isdefault=$('input[name="partnerrate"]').data('value');
    if(partnerrate<isdefault){
        Message.showMessage("合伙人提成不能低于平台默认提成的"+isdefault+"%!");
        return false
    }
       if(is_sure==4){
        if(!remarks){
