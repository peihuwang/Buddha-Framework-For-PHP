    ...
    function insertListDiv(json) {
        ...
        for (var i = 0; i< json.length; i++) {
            ...
            if(json[i].is_sure == "not" ){
                html+='  <span  data-href="index.php?a=edit&c=supply&id='+json[i].id+'">审 核 </span>';
            }else if(json[i].is_sure == "no" ){
                html+='  <span  data-href="index.php?a=edit&c=supply&id='+json[i].id+'">审 核 </span>';
            }else if (json[i].is_sure == "yes"){
                html+='  <span class="stop" id="state_'+json[i].id+'" data-u="'+json[i].user_id+'" data-value="'+json[i].id+'" data-a="isdel" data-c="supply">'+json[i].state+'</span>';
            }
            html+='  </div>';
            //////////////////////////////////
            html+='</div>';
...
function links(){
    ...
    var value=$(this).data('value'),
        ...
        txt=$(this).text(),
         s='';
        if(txt=='上 架'){
            s="现在产品为 [下 架] 状态,您确定要 ["+txt+"] 产品吗？";
        }else {
           s="现在产品为 [上 架] 状态,您确定要 ["+txt+"] 产品吗？</hr>";
        }
